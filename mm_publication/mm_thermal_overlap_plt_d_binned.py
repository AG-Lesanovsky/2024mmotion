...
T = 10.49  # Temperature (set your actual value)
                ...
                for line in lines:
                    ...
                    if -30010 <= E_adj <= -29990:  # Only consider energies in the desired range
                        if d not in data_dict:
                            data_dict[d] = []
                        data_dict[d].append((E_adj, nS * boltzmann_factor))  # Store energy and weighted overlap
...
for d, values in data_dict.items():
...
nS_trans = ax.scatter(binned_dx, binned_E, c=binned_nS, cmap='bone_r', s=5, rasterized=True, vmin=0, vmax=1, label=r'$\langle N \rangle = 10, T = 10.49$')
...
plt.text(10, -29995, r'(b) $\zeta = 10$', fontsize=24)
...
plt.tight_layout()
fig_save = f'C:\\Users\\Wilson\\OneDrive\\Dokumente\\2024mmotion\\1d_model\\finite_temp\\figures_finite\\thermal_z={eta}_T={T}.pdf'
